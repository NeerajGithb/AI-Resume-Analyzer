...
export default function SignupPage() {
  ...
  return (
    <div className="grid md:grid-cols-2 min-h-[calc(100vh-80px)]">
      {/* Left — form */}
      <div className="flex flex-col items-center justify-center p-8 bg-gray-50 order-2 md:order-1">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <ErrorMessage message={signupError?.message} />

            <Button type="submit" variant="primary" size="lg" className="w-full" loading={isSigningUp} disabled={isSigningUp}>
              Create Free Account
            </Button>
      ...
      <div className="hidden md:flex flex-col justify-between p-12 relative overflow-hidden order-1 md:order-2" style={{ background: 'linear-gradient(145deg,#0b0614 0%,#16062a 100%)' }}>
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-3xl font-extrabold text-white mb-3 leading-snug">
            Join 50,000+<br />professionals landing<br />
            <span className="bg-gradient-to-r from-violet-400 to-pink-400 bg-clip-text text-transparent">their dream jobs</span>
          </h2>
          ...
          <div className="space-y-4">
            {PERKS.map(({ icon, text }) => (
              <div key={text} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-sm bg-white/10 flex items-center justify-center text-lg shrink-0">{icon}</div>
        ...
        <div className="relative z-10 grid grid-cols-3 gap-4">
          {[{ val: '50k+', label: 'Users' }, { val: '94%', label: 'Score Boost' }, { val: '4.9★', label: 'Rating' }].map(({ val, label }) => (
            <div key={label} className="bg-white/8 rounded-sm p-4 text-center border border-white/10">
